...
export async function POST(request: Request){
    ...
    if(existingUser){
        return NextResponse.json({errors:["email already exists"], success: false}, {status: 400})
    }
    const user = await signUp(email,name,password);
    return NextResponse.json({ message: "User created!", success: true, user }, { status: 201 });
}
